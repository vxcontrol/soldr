...
var modulesSQLMappers = map[string]interface{}{
	"status":               "`{{table}}`.status",
	"system":               "`{{table}}`.system",
	"actions":              storage2.ActionsMapper,
	"events":               storage2.EventsMapper,
	"fields":               storage2.FieldsMapper,
	"tags":                 storage2.TagsMapper,
	"os":                   storage2.ModulesOSMapper,
	"os_arch":              storage2.ModulesOSArchMapper,
	"os_type":              storage2.ModulesOSTypeMapper,
...
func updatePolicyModulesByModuleS(c *gin.Context, moduleS *models.ModuleS, sv *models.Service) error {
	iDB := storage2.GetDB(sv.Info.DB.User, sv.Info.DB.Pass, sv.Info.DB.Host,
...
func (s *ModuleService) GetPolicyModules(c *gin.Context) {
	...
	queryA.Page = 0
	queryA.Size = 0
	queryA.Filters = []storage2.TableFilter{}
...
func (s *ModuleService) DeleteModule(c *gin.Context) {
	...
	deletePolicyModule := func(s *models.Service) error {
		...
		iDB := storage2.GetDB(s.Info.DB.User, s.Info.DB.Pass, s.Info.DB.Host,
...
func (s *ModuleService) CreateModuleVersion(c *gin.Context) {
	...
	switch semver2.CompareVersions(module.Info.Version.String(), version) {
	case semver2.TargetVersionGreat:
	default:
