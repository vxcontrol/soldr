...
type upgradesAgentsAction struct {
	Filters []storage2.TableFilter `form:"filters" json:"filters" binding:"omitempty"`
...
func (s *UpgradeService) CreateAgentsUpgrades(c *gin.Context) {
	...
	if sqlInsertResult.RowsAffected != 0 {
		...
		if err = uploadAgentBinariesToInstBucket(binary, s3); err != nil {
			logger.FromContext(c).WithError(err).Errorf("error uploading agent binaries to S3 instance bucket")
...
func (s *UpgradeService) PatchLastAgentUpgrade(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&task); err != nil || task.Valid() != nil {
		...
		name, nameErr := storage2.GetAgentName(iDB, hash)
		if nameErr == nil {
