...
func TestDecrypt(t *testing.T) {
	encryptor, err0 := NewAESEncryptor(generateTestKey)
	if err0 != nil {
		t.Fatalf("%s", err0)
	}

	prefix := "foo"
	dbencryptor := NewDBConfigEncryptor(encryptor, prefix)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if string(decrypted1) != string(decrypted1) {
				t.Fatalf("expected: %s; actual: %s", string(decrypted1), string(decrypted1))
			}
